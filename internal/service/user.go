...
type Audit interface {
	SenLogReq(ctx context.Context, req audit.LogItem) error
}
...
func NewUsers(repo UsersRepository, sessionsRepo SessionsRepository, hasher PasswordHasher, audit Audit, secret []byte) *Users {
	return &Users{
		repo:         repo,
...
func (s *Users) SignUp(ctx context.Context, inp domain.SignUpInput) error {
	...
	err = s.audit.SenLogReq(ctx, audit.LogItem{Action: audit.ACTION_REGISTER, Entity: audit.ENTITY_USER, EntityID: user.ID, Timestamp: time.Now()})
	if err != nil {
...
func (s *Users) SignIn(ctx context.Context, inp domain.SignInInput) (string, string, error) {
	...
	err = s.audit.SenLogReq(ctx, audit.LogItem{Action: audit.ACTION_LOGIN, Entity: audit.ENTITY_USER, EntityID: user.ID, Timestamp: time.Now()})
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"method": "Users.SignUp",
		}).Error("failed to send log request:", err)
